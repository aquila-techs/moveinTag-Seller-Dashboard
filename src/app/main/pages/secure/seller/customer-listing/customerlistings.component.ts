...
export class CustomerlistingsComponent implements OnInit {
  ...
  getAllQuotesOrders() {
    let queryParams =
      "?&status=QUOTE&searchByQuote=true&categoryId=" +
      this.categoryId +
      "&userId=" +
      this.userId +
      "&sortBy=createdAt&order=desc";
    this.orderService.getAllQuotesOrders(queryParams).subscribe((res) => {
  ...
  onSubmitSearch() {
    ...
    this.http
      .get(
        `https://api.moventag.com/order/searchSellerOrdersActiveLeads?status=QUOTE&searchByQuote=true&categoryId=${this.categoryId}&userId=${this.userId}&sortBy=createdAt&order=desc&text=${Text}`
      )
      .subscribe({
        next: (res: any) => {
          if (res.length < 1) {
  ...
  startChat(selectedOrder) {
    ...
    body.chatroom =
      emailString.split("").sort().join("") + "_" + selectedOrder._id;
    ...
    this.userService.createChatRoom(body).subscribe({
      next: (res) => {
        if (selectedOrder.buyer._id) {
          let data = {
            heading: "You have got new message",
