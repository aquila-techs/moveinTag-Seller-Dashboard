...
export class GoogleMapsService {
  ...
  public getZipCodesInRadius(zipCode: string, radius: number): Promise<string[]> {
  ...
  public findNearbyZipcodes(lat: number, lng: number, radius: number): Promise<string[]> {
    const url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=${radius}&type=postal_code&key=AIzaSyAgx1yEOwP_pASKnrOidhM6H-fUoTEyDu0`;
